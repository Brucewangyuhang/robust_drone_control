...
class Quadrotor(BaseAviary):
    ...
    def __init__(self,
                 ...
                 path_reference_window=25,
                 path_reference_lookahead=5,
                 path_reference_search_behind=3,
                 path_projection_max_advance=None,
                 path_reference_init='nearest',
                 path_tracking_reward_weight=1.0,
                 path_progress_weight=5.0,
                 path_progress_clip=0.05,
                 path_progress_gate_scale=0.5,
                 path_error_threshold=0.25,
                 path_danger_penalty=0.0,
                 path_danger_time_penalty=0.0,
                 path_completion_threshold=0.98,
                 path_completion_error_threshold=None,
                 path_completion_bonus=0.0,
                 done_on_path_completion=False,
                 recovery_error_tolerance=0.1,
                 **kwargs
                 ):
        ...
        Args:
            ...
            tracking_reference_mode (str): `time_indexed` for the original strict tracking task,
                `path_following` to project onto the path and use a mergeable forward reference.
            path_reference_window (int): Number of future waypoints considered when projecting onto the path.
            path_reference_lookahead (int): Number of waypoints to look ahead after projecting onto the path.
            path_reference_search_behind (int): Small backward search window to avoid projection chatter.
            path_projection_max_advance (int): Maximum waypoint advance allowed in a single projection update.
            path_reference_init (str): How to initialize path progress, either `nearest` or `start`.
            path_tracking_reward_weight (float): Weight on the dense tracking reward in path-following mode.
            path_progress_weight (float): Reward bonus applied to forward path progress.
            path_progress_clip (float): Maximum normalized progress increment rewarded in one control step.
            path_progress_gate_scale (float): Exponent applied to the hard path-error gate for progress reward.
            path_error_threshold (float): Distance threshold that marks a danger-zone tracking deviation.
            path_danger_penalty (float): Quadratic deviation penalty weight beyond `path_error_threshold`.
            path_danger_time_penalty (float): Per-step reward penalty for staying outside the safe tracking tube.
            path_completion_threshold (float): Normalized progress from start that marks figure-8 completion.
            path_completion_error_threshold (float): Path-error tolerance required to mark completion.
            path_completion_bonus (float): One-time reward bonus when `path_completion_threshold` is reached.
            done_on_path_completion (bool): Whether to terminate an episode after completing the path.
            recovery_error_tolerance (float): Error tolerance used to mark a disturbance recovery event.
        '''

        custom_init_state_randomization_info = deepcopy(kwargs.get('init_state_randomization_info', None))
        ...
        self.tracking_reference_mode = str(tracking_reference_mode).lower()
        if self.tracking_reference_mode not in {'time_indexed', 'path_following'}:
            raise ValueError('[ERROR] in Quadrotor.__init__(), tracking_reference_mode must be `time_indexed` or `path_following`.')
        self.path_reference_window = max(1, int(path_reference_window))
        self.path_reference_lookahead = max(0, int(path_reference_lookahead))
        self.path_reference_search_behind = max(0, int(path_reference_search_behind))
        self.path_projection_max_advance = None
        if path_projection_max_advance is not None:
            self.path_projection_max_advance = max(1, int(path_projection_max_advance))
        self.path_reference_init = str(path_reference_init).lower()
        if self.path_reference_init not in {'nearest', 'start'}:
            raise ValueError('[ERROR] in Quadrotor.__init__(), path_reference_init must be `nearest` or `start`.')
        self.path_tracking_reward_weight = float(path_tracking_reward_weight)
        self.path_progress_weight = float(path_progress_weight)
        self.path_progress_clip = max(0.0, float(path_progress_clip))
        self.path_progress_gate_scale = max(0.0, float(path_progress_gate_scale))
        self.path_error_threshold = float(path_error_threshold)
        self.path_danger_penalty = float(path_danger_penalty)
        self.path_danger_time_penalty = max(0.0, float(path_danger_time_penalty))
        self.path_completion_threshold = float(path_completion_threshold)
        if path_completion_error_threshold is None:
            self.path_completion_error_threshold = self.path_error_threshold
        else:
            self.path_completion_error_threshold = float(path_completion_error_threshold)
        self.path_completion_bonus = float(path_completion_bonus)
        self.done_on_path_completion = bool(done_on_path_completion)
        self.recovery_error_tolerance = float(recovery_error_tolerance)
        ...
        if custom_init_state_randomization_info is not None:
            self.INIT_STATE_RAND_INFO.update(custom_init_state_randomization_info)
        ...
        if self.TASK == Task.STABILIZATION:
            ...
            self._setup_path_reference_data()
    ...
    def reset(self, seed=None):
        ...
        self.path_projection_index = None
        self.path_progress = 0.0
        self.path_progress_from_start = 0.0
        self.path_progress_delta = 0.0
        self.path_error = 0.0
        self.path_start_progress = 0.0
        self.path_completed = False
        self.path_completion_reward_given = False
        self.current_reference_index = 0
        self.current_reference_state = np.array(self.X_GOAL[0], copy=True) if self.TASK == Task.TRAJ_TRACKING else np.array(self.X_GOAL, copy=True)
        self.last_applied_disturbance = np.zeros(3, dtype=float)
    ...
    def step(self, action):
        ...
        if disturb_force is not None:
            ...
            self.last_applied_disturbance = np.asarray(disturb_force, dtype=float)
        else:
            self.last_applied_disturbance = np.zeros(3, dtype=float)
    ...
    def _get_position_indices(self):
        '''Indices of the position states for the current quadrotor model.'''
    ...
    def _setup_path_reference_data(self):
        '''Precompute geometric path data used by path-following rewards and metrics.'''

        self.path_reference_positions = self.X_GOAL[:, self._get_position_indices()]
        segment_lengths = np.linalg.norm(np.diff(self.path_reference_positions, axis=0), axis=1)
        self.path_reference_cum_lengths = np.concatenate([[0.0], np.cumsum(segment_lengths)])
        self.path_reference_total_length = float(self.path_reference_cum_lengths[-1])
        if self.path_reference_total_length <= 0:
            self.path_reference_total_length = 1.0

    def _path_progress_from_index(self, ref_idx):
        '''Normalized progress along the reference path for a given waypoint index.'''

        if self.TASK != Task.TRAJ_TRACKING:
            return 0.0
        return float(self.path_reference_cum_lengths[ref_idx] / self.path_reference_total_length)

    def _project_to_path(self, state, search_from_scratch=False):
        '''Project the current position onto the geometric path using a local forward search.'''

        pos = np.asarray(state)[self._get_position_indices()]
        last_idx = 0 if self.path_projection_index is None else int(self.path_projection_index)
        if search_from_scratch and self.path_reference_init == 'nearest':
            search_start = 0
            search_end = self.X_GOAL.shape[0] - 1
        else:
            search_start = max(0, last_idx - self.path_reference_search_behind)
            search_window = self.path_reference_window
            if self.path_projection_max_advance is not None:
                search_window = min(search_window, self.path_projection_max_advance)
            search_end = min(self.X_GOAL.shape[0] - 1, last_idx + search_window)
        candidate_idx = np.arange(search_start, search_end + 1, dtype=int)
        ref_positions = self.path_reference_positions[candidate_idx]
        path_error = np.linalg.norm(ref_positions - pos, axis=1)
        best_idx = int(candidate_idx[np.argmin(path_error)])
        return best_idx, float(np.min(path_error))

    def _update_tracking_reference(self):
        '''Update path projection metrics and the controller-facing reference state.'''

        if self.TASK != Task.TRAJ_TRACKING:
            self.current_reference_index = 0
            self.current_reference_state = np.array(self.X_GOAL, copy=True)
            return

        projected_idx, path_error = self._project_to_path(
            self.state,
            search_from_scratch=self.path_projection_index is None,
        )
        previous_progress = self.path_progress
        if self.path_projection_index is not None:
            projected_idx = max(projected_idx, int(self.path_projection_index))
            pos = np.asarray(self.state)[self._get_position_indices()]
            path_error = float(np.linalg.norm(self.path_reference_positions[projected_idx] - pos))
        current_progress = self._path_progress_from_index(projected_idx)

        if self.path_projection_index is None:
            self.path_start_progress = current_progress
            self.path_progress_delta = 0.0
        else:
            self.path_progress_delta = max(0.0, current_progress - previous_progress)

        self.path_projection_index = projected_idx
        self.path_progress = current_progress
        self.path_progress_from_start = max(0.0, current_progress - self.path_start_progress)
        self.path_error = path_error
        self.path_completed = (
            self.path_progress_from_start >= self.path_completion_threshold
            and self.path_error <= self.path_completion_error_threshold
        )

        if self.tracking_reference_mode == 'path_following':
            ref_idx = min(projected_idx + self.path_reference_lookahead, self.X_GOAL.shape[0] - 1)
        else:
            ref_idx = min(self.ctrl_step_counter + 1, self.X_GOAL.shape[0] - 1)
        self.current_reference_index = int(ref_idx)
        self.current_reference_state = np.array(self.X_GOAL[self.current_reference_index], copy=True)

    def get_obs_goal_horizon_indices(self, next_step):
        '''Reference indices appended to the RL observation horizon.'''

        if self.tracking_reference_mode != 'path_following' or self.TASK != Task.TRAJ_TRACKING:
            return [
        ...
        return [
            min(self.current_reference_index + i, self.X_GOAL.shape[0] - 1)
            for i in range(self.obs_goal_horizon)
        ]
    ...
    def _get_observation(self):
        ...
        if self.TASK == Task.TRAJ_TRACKING:
            self._update_tracking_reference()
    ...
    def _get_reward(self):
        ...
        if self.COST == Cost.RL_REWARD:
            ...
            if self.TASK == Task.TRAJ_TRACKING:
                state_error = state - self.current_reference_state
                dist = np.sum(self.rew_state_weight * state_error * state_error)
                dist += np.sum(self.rew_act_weight * act_error * act_error)
                if self.tracking_reference_mode == 'path_following':
                    if self.path_danger_penalty > 0:
                        deviation = max(0.0, self.path_error - self.path_error_threshold)
                        dist += self.path_danger_penalty * deviation ** 2
            rew = -dist
            # Convert rew to be positive and bounded [0,1].
            if self.rew_exponential:
                rew = np.exp(rew)
            if self.TASK == Task.TRAJ_TRACKING and self.tracking_reference_mode == 'path_following':
                rew *= self.path_tracking_reward_weight
                progress_delta = np.clip(self.path_progress_delta, 0.0, self.path_progress_clip)
                if self.path_error_threshold > 0:
                    progress_gate = max(0.0, 1.0 - self.path_error / self.path_error_threshold)
                    if self.path_progress_gate_scale > 0:
                        progress_gate = progress_gate ** self.path_progress_gate_scale
                else:
                    progress_gate = np.exp(-self.path_progress_gate_scale * self.path_error)
                rew += self.path_progress_weight * progress_delta * progress_gate
                if self.path_danger_time_penalty > 0 and self.path_error > self.path_error_threshold:
                    deviation_ratio = (self.path_error - self.path_error_threshold) / max(1e-6, self.path_error_threshold)
                    rew -= self.path_danger_time_penalty * (1.0 + deviation_ratio)
                if self.path_completed and not self.path_completion_reward_given:
                    rew += self.path_completion_bonus
                    self.path_completion_reward_given = True
            return rew
        ...
        if self.COST == Cost.QUADRATIC:
            ...
            if self.TASK == Task.TRAJ_TRACKING:
                return float(-1 * self.symbolic.loss(x=self.state,
                                                     Xr=self.current_reference_state,
                                                     u=self.current_clipped_action,
                                                     Ur=self.U_GOAL,
                                                     Q=self.Q,
                                                     R=self.R)['l'])
    ...
    def _get_done(self):
        ...
        if self.TASK == Task.TRAJ_TRACKING and self.tracking_reference_mode == 'path_following':
            if self.done_on_path_completion and self.path_completed:
                return True
    ...
    def _get_info(self):
        ...
        if self.TASK == Task.TRAJ_TRACKING:
            info['tracking_reference_mode'] = self.tracking_reference_mode
            info['tracking_reference_index'] = self.current_reference_index
            info['path_error'] = self.path_error
            info['path_progress'] = self.path_progress
            info['path_progress_from_start'] = self.path_progress_from_start
            info['path_progress_delta'] = self.path_progress_delta
            info['path_completed'] = float(self.path_completed)
            info['path_completion_error_threshold'] = self.path_completion_error_threshold
            info['in_danger_zone'] = float(self.path_error > self.path_error_threshold)
            disturbance_norm = float(np.linalg.norm(self.last_applied_disturbance))
            info['disturbance_norm'] = disturbance_norm
            info['disturbance_active'] = float(disturbance_norm > 1e-9)
            info['recovery_target_reached'] = float(self.path_error <= self.recovery_error_tolerance)
        return info
