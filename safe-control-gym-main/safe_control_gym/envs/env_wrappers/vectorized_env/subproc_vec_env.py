...
def worker(remote, parent_remote, env_fn_wrappers):
    ...
    def sanitize_info(info):
        '''Remove non-pickleable reset metadata before crossing process boundaries.'''
    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                remote.send([(obs, sanitize_info(info)) for obs, info in [env.reset() for env in envs]])
            elif cmd == 'render':
