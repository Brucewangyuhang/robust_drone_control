                ...
                for _ in range(sim_steps):
                    ...
                    if done:
                        ...
                        if trajs < n_episodes and seeds is not None:
                            seed = seeds[trajs]
                        self.env.save_data()
                        obs, info = self._evaluation_reset(ctrl_data=ctrl_data, sf_data=sf_data, seed=seed)
                        break
...
class MetricExtractor:
    ...
    * episode constraint violation steps
    ...
    def compute_metrics(self, data, verbose=False):
        ...
        if self.has_episode_data('path_error'):
            episode_path_error = self.get_episode_path_error()
            episode_peak_path_error = self.get_episode_peak_path_error()
            metrics.update({
                'average_path_error': np.asarray(episode_path_error).mean(),
                'path_error': np.asarray(episode_path_error) if len(episode_path_error) > 1 else episode_path_error[0],
                'average_peak_path_error': np.asarray(episode_peak_path_error).mean(),
                'peak_path_error': np.asarray(episode_peak_path_error) if len(episode_peak_path_error) > 1 else episode_peak_path_error[0],
            })

        if self.has_episode_data('path_progress_from_start'):
            episode_progress = self.get_episode_progress()
            metrics.update({
                'average_final_progress': np.asarray(episode_progress).mean(),
                'final_progress': np.asarray(episode_progress) if len(episode_progress) > 1 else episode_progress[0],
            })

        if self.has_episode_data('path_completed'):
            episode_completed = self.get_episode_path_completed()
            metrics.update({
                'completion_rate': np.asarray(episode_completed).mean(),
                'path_completed': np.asarray(episode_completed) if len(episode_completed) > 1 else episode_completed[0],
            })

        if self.has_episode_data('in_danger_zone'):
            danger_zone_steps = self.get_episode_danger_zone_steps()
            metrics.update({
                'average_danger_zone_steps': np.asarray(danger_zone_steps).mean(),
                'danger_zone_steps': np.asarray(danger_zone_steps) if len(danger_zone_steps) > 1 else danger_zone_steps[0],
            })

        if self.has_episode_data('disturbance_active') and self.has_episode_data('path_error'):
            recovery_times = np.asarray(self.get_episode_recovery_times(), dtype=float)
            peak_deviation = np.asarray(self.get_episode_peak_deviation_after_disturbance(), dtype=float)
            if np.any(np.isfinite(recovery_times)):
                metrics.update({
                    'average_recovery_time': float(np.nanmean(recovery_times)),
                    'recovery_time': recovery_times if len(recovery_times) > 1 else recovery_times[0],
                })
            if np.any(np.isfinite(peak_deviation)):
                metrics.update({
                    'average_peak_deviation_after_disturbance': float(np.nanmean(peak_deviation)),
                    'peak_deviation_after_disturbance': peak_deviation if len(peak_deviation) > 1 else peak_deviation[0],
                })
        return metrics

    def has_episode_data(self, key):
        '''Whether a field exists either as top-level data or in step info dicts.'''
    ...
    def get_episode_data(self, key, postprocess_func=lambda x: x):
        ...
        elif self.has_episode_data(key):
    ...
    def get_episode_path_error(self):
        '''Mean geometric path error over an episode.'''

        return self.get_episode_data('path_error',
                                     postprocess_func=lambda x: float(np.mean(x)))

    def get_episode_peak_path_error(self):
        '''Peak geometric path error over an episode.'''

        return self.get_episode_data('path_error',
                                     postprocess_func=lambda x: float(np.max(x)))

    def get_episode_progress(self):
        '''Final progress along the reference path relative to the episode start.'''

        return self.get_episode_data('path_progress_from_start',
                                     postprocess_func=lambda x: float(x[-1]) if len(x) > 0 else 0.0)

    def get_episode_danger_zone_steps(self):
        '''Counts steps where the trajectory leaves the configured safe tracking tube.'''

        return self.get_episode_data('in_danger_zone',
                                     postprocess_func=lambda x: float(np.sum(x)))

    def get_episode_path_completed(self):
        '''Whether an episode completed the geometric path.'''

        return self.get_episode_data('path_completed',
                                     postprocess_func=lambda x: float(np.any(x)))

    def get_episode_recovery_times(self):
        '''Time from the first disturbance to the first recovery event in each episode.'''

        recovery_times = []
        for ep_info in self.data['info']:
            step_info = [info for info in ep_info if 'path_error' in info]
            disturbance_step = None
            for step, info in enumerate(step_info):
                if info.get('disturbance_active', 0):
            ...
            if disturbance_step is None:
                ...
                continue
            recovered_step = None
            for step in range(disturbance_step, len(step_info)):
                if step_info[step].get('recovery_target_reached', 0):
                    recovered_step = step
                    break
            if recovered_step is None:
                recovery_times.append(np.nan)
            else:
                recovery_times.append(float(recovered_step - disturbance_step))
        return recovery_times

    def get_episode_peak_deviation_after_disturbance(self):
        '''Peak path deviation measured after the first disturbance event.'''
        ...
        for ep_info in self.data['info']:
            ...
            path_error = []
            for step, info in enumerate(step_info):
                path_error.append(float(info['path_error']))
                if disturbance_step is None and info.get('disturbance_active', 0):
                    disturbance_step = step
            if disturbance_step is None or not path_error:
                peak_deviation.append(np.nan)
            else:
                peak_deviation.append(float(np.max(path_error[disturbance_step:])))
