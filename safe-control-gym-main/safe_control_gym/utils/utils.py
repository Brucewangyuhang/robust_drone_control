...
def get_random_state():
    ...
    return {
        ...
        'torch': torch.get_rng_state().cpu().numpy().copy()
    }
...
def set_random_state(state_dict):
    ...
    torch.torch.set_rng_state(torch_state.cpu())
...
def set_device_from_config(config):
    '''Sets device, preferring CUDA then Apple MPS when accelerator use is requested.'''
