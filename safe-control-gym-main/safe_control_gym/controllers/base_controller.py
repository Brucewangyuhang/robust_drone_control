...
class BaseController(ABC):
    ...
    def __init__(self,
        ...
        Args:
            ...
            use_gpu (bool): False (use cpu) True (use cuda/mps when available).
        ...
        self.device = self._select_device(self.use_gpu)
        self.use_gpu = self.device.type in ['cuda', 'mps']

        self.setup_results_dict()

    def _select_device(self, use_accelerator):
        '''Selects the best available torch accelerator.'''

        if use_accelerator and torch.cuda.is_available():
            return torch.device('cuda')
        if use_accelerator and hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
