...
class PPOAgent:
    ...
    def load_state_dict(self,
        ...
        self._optimizer_to_device(self.actor_opt, device)
        self._optimizer_to_device(self.critic_opt, device)

    def _optimizer_to_device(self, optimizer, device):
        '''Moves optimizer tensors to the same device as the model after checkpoint load.'''
...
class PPOBuffer(object):
    ...
    def get(self,
        ...
        for k, info in self.scheme.items():
            ...
            batch[k] = torch.as_tensor(data, dtype=torch.float32, device=device)
        return batch
    ...
    def sampler(self,
        ...
        for indices in sampler:
            ...
            batch = {
                k: torch.as_tensor(v, dtype=torch.float32, device=device) for k, v in batch.items()
            }
            yield batch
...
def compute_returns_and_advantages(rews,
    ...
    T, N = rews.shape[:2]
    rews = np.asarray(rews, dtype=np.float32).copy()
    vals = np.asarray(vals, dtype=np.float32)
    masks = np.asarray(masks, dtype=np.float32)
    terminal_vals = np.asarray(terminal_vals, dtype=np.float32)
    last_val = np.asarray(last_val, dtype=np.float32)
    if terminal_vals.shape == ():
        terminal_vals = np.full((T, N, 1), terminal_vals, dtype=np.float32)
    if last_val.shape == ():
        last_val = np.full((N, 1), last_val, dtype=np.float32)
    elif last_val.shape == (N,):
        last_val = last_val.reshape(N, 1)
    rets = np.zeros((T, N, 1), dtype=np.float32)
    advs = np.zeros((T, N, 1), dtype=np.float32)
    ret, adv = last_val.copy(), np.zeros((N, 1), dtype=np.float32)
    ...
    for i in reversed(range(T)):
        ...
        rets[i] = deepcopy(ret)
        advs[i] = deepcopy(adv)
    return rets.astype(np.float32), advs.astype(np.float32)
