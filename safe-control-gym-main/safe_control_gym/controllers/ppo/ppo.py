...
class PPO(BaseController):
    ...
    def _add_path_following_trackers(self, env):
        '''Adds optional robust path-following metrics to episode summaries.'''

        for key in ['path_error', 'path_progress_delta', 'path_completed', 'in_danger_zone', 'disturbance_active']:
            env.add_tracker(key, 0, mode='queue')

    def _mean_or_nan(self, values):
        '''Mean helper that stays quiet when no episode has finished yet.'''
    ...
    def reset(self):
        ...
        if self.training:
            ...
            self._add_path_following_trackers(self.env)
            self.eval_env.add_tracker('constraint_violation', 0, mode='queue')
            self.eval_env.add_tracker('mse', 0, mode='queue')
            self._add_path_following_trackers(self.eval_env)
    ...
    def save(self,
        ...
        if self.training:
            ...
            if hasattr(self, 'eval_best_score'):
                exp_state['eval_best_score'] = self.eval_best_score
    ...
    def load(self,
        ...
        state = torch.load(path, map_location='cpu', weights_only=False)  # Safe since we're loading our own models
        # Restore policy.
        ...
        if self.training:
            ...
            self.eval_best_score = state.get('eval_best_score', -np.inf)
    ...
    def learn(self,
        ...
        if self.num_checkpoints > 0:
            ...
            interval_save = step_interval <= self.total_steps
        next_save_step = None
        if self.save_interval:
            next_save_step = (self.total_steps // self.save_interval + 1) * self.save_interval
        next_eval_step = None
        if self.eval_interval:
            next_eval_step = (self.total_steps // self.eval_interval + 1) * self.eval_interval
        next_log_step = None
        if self.log_interval:
            next_log_step = (self.total_steps // self.log_interval + 1) * self.log_interval
        while self.total_steps < self.max_env_steps:
            results = self.train_step()
            # Checkpoint.
            should_save = self.total_steps >= self.max_env_steps
            if next_save_step is not None and self.total_steps >= next_save_step:
                should_save = True
                while next_save_step <= self.total_steps:
            ...
            # Evaluation.
            should_eval = next_eval_step is not None and self.total_steps >= next_eval_step
            if should_eval:
            ...
            # Logging.
            should_log = next_log_step is not None and self.total_steps >= next_log_step
            if should_log:
                while next_log_step <= self.total_steps:
                    next_log_step += self.log_interval
    ...
    def run(self,
        ...
        if env is None:
            ...
            if not is_wrapped(env, RecordEpisodeStatistics):
                ...
                self._add_path_following_trackers(env)
    ...
    def train_step(self):
        ...
        rollouts.ret = ret.astype(np.float32)
        # Prevent divide-by-0 for repetitive tasks.
        rollouts.adv = ((adv - adv.mean()) / (adv.std() + 1e-6)).astype(np.float32)
    ...
    def log_step(self,
        ...
        ep_reward = ep_returns / ep_lengths if ep_lengths.size > 0 else np.asarray([])
        stat_scalars = {
            'ep_length': self._mean_or_nan(ep_lengths),
            'ep_return': self._mean_or_nan(ep_returns),
            'ep_reward': self._mean_or_nan(ep_reward),
            'ep_constraint_violation': self._mean_or_nan(ep_constraint_violation)
        }
        if 'path_error' in self.env.queued_stats and len(self.env.queued_stats['path_error']) > 0:
            ep_path_error = np.asarray(self.env.queued_stats['path_error'])
            ep_final_progress = np.asarray(self.env.queued_stats['path_progress_delta'])
            ep_path_completed = np.asarray(self.env.queued_stats.get('path_completed', []))
            ep_danger_steps = np.asarray(self.env.queued_stats['in_danger_zone'])
            ep_disturbance_steps = np.asarray(self.env.queued_stats['disturbance_active'])
            stat_scalars.update({
                'ep_path_error': self._mean_or_nan(ep_path_error / ep_lengths),
                'ep_final_progress': self._mean_or_nan(ep_final_progress),
                'ep_completion_rate': self._mean_or_nan(ep_path_completed > 0),
                'ep_danger_zone_steps': self._mean_or_nan(ep_danger_steps),
                'ep_disturbance_steps': self._mean_or_nan(ep_disturbance_steps),
            })
        self.logger.add_scalars(stat_scalars, step, prefix='stat')
        ...
        if 'eval' in results:
            ...
            if 'path_error' in results['eval']:
                stat_eval_scalars.update({
                    'path_error': (results['eval']['path_error'] / eval_ep_lengths).mean(),
                    'final_progress': results['eval']['path_progress_delta'].mean(),
                    'completion_rate': (results['eval'].get('path_completed', np.asarray([])) > 0).mean(),
                    'danger_zone_steps': results['eval']['in_danger_zone'].mean(),
                    'disturbance_steps': results['eval']['disturbance_active'].mean(),
                })
            self.logger.add_scalars(stat_eval_scalars, step, prefix='stat_eval')
        # Print summary table
